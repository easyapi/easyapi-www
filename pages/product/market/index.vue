<template>
  <div>
    <div class="u-banner-bg">
      <div class="content">
        <div class="sysinfo f-tac">
          <h1>API服务市场</h1>
          <p>为您的企业搭建API服务市场，建立自己的开放合作的行业生态圈</p>
          <a href="/system/order/add" class="bug_btn">立刻部署API服务市场</a>
          <div class="more">
            独立第三方API服务市场解决方案提供商
          </div>
        </div>
      </div>
    </div>

    <div class="pd-100" style="padding-bottom: 0">
      <div class="title">最专业API服务市场供应商</div>
      <div class="description">我们是专一的API团队，我们有成熟的API服务市场运营经验 <br>
        基于我们的经验，为您的公司快速架构API服务市场，助力贵公司行业生态建设
      </div>
      <div class="u-igateway f-cb">
        <div style="width:600px;margin-right: 0;height:400px" class="f-fr img">
          <img style="width: 100%" src="/images/info/market_img1.png">
        </div>

        <div class="text f-fl" style="margin-top: 0px;">
          <p style="color: #000;font-size: 24px;">服务市场门户</p>
          <p>
            <img style="width: 6px;height: 6px;" src="/images/info/market_tag.png">
            实现API服务分类浏览，API服务搜索<br>
            <img style="width: 6px;height: 6px;" src="/images/info/market_tag.png">了解API服务详情，API接口信息

          </p>
        </div>
      </div>
    </div>


    <div class="pd-100" style="background-color: #eee;padding-bottom: 0">

      <div class="u-igateway f-cb">
        <div style="width: 600px;margin-right: 0;height:400px;" class="f-fl img">
          <img style="width: 100%;" src="/images/info/market_img2.png">
        </div>
        <div class="text f-fr" style="margin-top: 0px;">
          <p style="color: #000;font-size: 24px;">开发者中心</p>
          <p><img style="width: 6px;height: 6px;" src="/images/info/market_tag.png" alt="">申请API服务，创建应用，获取密钥信息
            <br>
            <img style="width: 6px;height: 6px;" src="/images/info/market_tag.png" alt="">API服务充值与续费，API调用请求明细
          </p>
        </div>
      </div>
    </div>

    <div class="pd-100" style="padding-bottom: 0">
      <div class="u-igateway f-cb">
        <div style="width: 600px;margin-right: 0;height:400px;" class="f-fr img">
          <img style="width: 100%;margin-top: -6px;" src="/images/info/market_img3.png">
        </div>
        <div class="text f-fl" style="margin-top: 0px;">
          <p style="color: #000;font-size: 24px;">服务商管理中心</p>
          <p><img style="width: 6px;height: 6px;" src="/images/info/market_tag.png">申请成为平台服务商，实现API服务发布
            <br>
            <img style="width: 6px;height: 6px;" src="/images/info/market_tag.png">了解自己的API服务销售业务开展情况

          </p>
        </div>
      </div>
    </div>

    <div class="u-infofoot">
      <h3>想了解更多的内容</h3>
      <div class="codebox">
        <div class="code f-fl">
          <img width="120" src="/images/info/leida_wechat.png">
          <p>微信联系</p>
        </div>
        <div class="code f-fr">
          <img width="120" src="/images/info/leida_qq.png">
          <p>QQ联系</p>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import Index from './index'

export default Index
</script>
